...
if (HAS_TOURNIQUET_APPLIED_ON(_target,_partIndex)) then {
    _actionData set [2, format [QACEPATHTOF(medical_gui,ui\cross_t_%1.paa), _colorInt]];
} else {
    _actionData set [2, [QACEPATHTOF(medical_gui,ui\cross.paa), COLOR_SCALE select _colorInt]];
};