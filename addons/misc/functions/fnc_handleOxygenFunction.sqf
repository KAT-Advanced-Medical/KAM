...
private _demandVentilation = ((((_actualHeartRate * 60) / _anerobicPressure) + (((_bloodGas select 0) - 40) * 200)) max 2000);
...
private _actualVentilation = (_tidalVolume * _respiratoryRate) max 1;
...
private _previousCycleSpo2 = _bloodGas select 1;

private _pao2 = ((90 - (((_unit getVariable [QEGVAR(circulation,ECB), 2700]) max 100) / 2700)) - ((_demandVentilation - _actualVentilation) / 120)) min _pALVo2;

_pao2 = if (_previousCycleSpo2 != _pao2) then { ([ _previousCycleSpo2 - (0.1 * _deltaT) , _previousCycleSpo2 + (0.1 * _deltaT)] select ((_previousCycleSpo2 - _pao2) < 0)) } else { _pao2 };
...
_unit setVariable [QEGVAR(breathing,etco2), _etco2, _syncValues];
_unit setVariable [QEGVAR(circulation,bloodGas), [_paco2, _pao2, _o2Sat, 24, _pH, _etco2], _syncValues];
