...
if (_maxDosage > 0) then {
    ...
    if (_currentDose >= floor (_maxDosage + floor(random(1)))) then {
...
// Check incompatible medication (format [med,limit])
{
    ...
    private _inSystem = [_target, _xMed] call ACEFUNC(medical_status,getMedicationCount);
...
if (_overdosedMedications isNotEqualTo []) then {
    private _medicationConfig = (configFile >> QUOTE(ACE_ADDON(Medical_Treatment)) >> "Medication");
    ...
    if (_onOverDose == "") exitWith {
        ...
        [QACEGVAR(medical,CriticalVitals), _target] call CBA_fnc_localEvent;
    };
