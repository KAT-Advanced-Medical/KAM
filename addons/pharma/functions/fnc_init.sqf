...
if (GVAR(coagulation)) then {
    ...
    [{
        ...
        if (_currentCoagFactors == _savedCoagFactors && _currentCoagFactors < _limitRegenCoagFactors) exitWith {
            ...
            _unit setVariable [QGVAR(coagulationFactor), (_currentCoagFactors + _amountToAdd), true];
            _unit setVariable [QGVAR(coagulationSavedFactors), (_currentCoagFactors + _amountToAdd), true];
        };
