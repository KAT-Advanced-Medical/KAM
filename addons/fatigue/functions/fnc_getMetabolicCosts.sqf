...
private _duty = ACEGVAR(advanced_fatigue,animDuty);
...
} forEach (values ACEGVAR(advanced_fatigue,dutyList));

// Metabolic cost for walking and running is different
if (_speed > 2) then {
    // Running
    #ifdef DEBUG_MODE_FULL
    private _baseline = 2.1 * _bodyMass + 4 * (_bodyMass + _gearMass) * ((_gearMass / _bodyMass) ^ 2) + (_bodyMass + _gearMass) * 0.9 * (_speed ^ 2);
    private _graded = 2.1 * _bodyMass + 4 * (_bodyMass + _gearMass) * ((_gearMass / _bodyMass) ^ 2) + _terrainFactor * (_bodyMass + _gearMass) * (0.9 * (_speed ^ 2) + 0.66 * _speed * _terrainGradient);
    ...
    (
        2.1 * _bodyMass
        + 4 * (_bodyMass + _gearMass) * ((_gearMass / _bodyMass) ^ 2)
        + _terrainFactor * (_bodyMass + _gearMass) * (0.9 * (_speed ^ 2) + 0.66 * _speed * _terrainGradient)
    ) * BIOMECH_EFFICIENCY * _duty
} else {
    // Walking
    #ifdef DEBUG_MODE_FULL
    private _baseline = 1.05 * _bodyMass + 2 * (_bodyMass + _gearMass) * ((_gearMass / _bodyMass) ^ 2) + (_bodyMass + _gearMass) * 1.15 * (_speed ^ 2);
    private _graded = 1.05 * _bodyMass + 2 * (_bodyMass + _gearMass) * ((_gearMass / _bodyMass) ^ 2) + _terrainFactor * (_bodyMass + _gearMass) * (1.15 * (_speed ^ 2) + 0.66 * _speed * _terrainGradient);
    private _terrainImpact = abs ((_graded / _baseline) - 1);
    hintSilent format ["FwdAngle: %1 | SideAngle: %2 \n TerrainFactor: %3 | TerrainGradient: %4 \n TerrainImpact: %5 \n Speed: %6 | CarriedLoad: %7 \n Duty: %8 | Work: %9",
        _fwdAngle toFixed 1,
        _sideAngle toFixed 1,
    ...
    (
        1.05 * _bodyMass
        + 2 * (_bodyMass + _gearMass) * ((_gearMass / _bodyMass) ^ 2)
        + _terrainFactor * (_bodyMass + _gearMass) * (1.15 * (_speed ^ 2) + 0.66 * _speed * _terrainGradient)
    ) * BIOMECH_EFFICIENCY * _duty
};