...
_condition && !(_patient getVariable [QGVAR(DefibrillatorPads_Connected), false]) && !(_patient getVariable [QEGVAR(airway,recovery), false]) && (objectParent _patient == objectParent _medic) && {["",_patient] call ACEFUNC(medical_treatment,canCPR)};
