...
switch (_AEDOrigin) do {
    case 1: {
        ...
        _condition = ((_patient distance2D _placedAED) < GVAR(Defibrillator_DistanceLimit)) && (_placedAED getVariable [QGVAR(AED_X_VitalsMonitor_Patient), objNull] in [objNull, _patient] || ((_patient getVariable [QGVAR(AED_X_VitalsMonitor_Provider), [objNull, -1, -1]]) select 0) isEqualTo _placedAED);
    };
    case 2: {
