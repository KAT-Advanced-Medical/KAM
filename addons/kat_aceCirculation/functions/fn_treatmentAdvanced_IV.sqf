...
_counts = _unit getVariable ["kat_aceCirculation_IV_counts", 0];
if (_counts isEqualTo 0) then {_counts = 1};
_ppEffect = (0.01 * _counts);
...
_volume = getNumber (configFile >> "ACE_Medical_Advanced" >> "Treatment" >> "IV" >> _className >> "volume");
_hradjust = _volume / 25;
[_unit, _hradjust, 60] call ace_medical_fnc_addHeartRateAdjustment;

//todo tod?
_a = (_unit getVariable ["kat_aceCirculation_IV_counts", 0]) + 1;
