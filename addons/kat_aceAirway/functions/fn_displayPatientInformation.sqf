...
ace_medical_currentSelectedSelectionN = [0, _selectionN] select (IS_SCALAR(_selectionN));
...
if (_show) then {
    ...
    [{
        ...
        if (((_target getVariable ["ace_medical_tourniquets", [0,0,0,0,0,0]]) select _selectionN) > 0) then {
            ...
            _genericMessages pushback ["HÖHÖHÖHHÖHÖHÖH", [0.77, 0.51, 0.08, 1]];
        };
        ...
        // EDIT by Katalam
        if ((_target getVariable["kat_aceAirway_airway", false]) && (_partText == "STR_ACE_medical_Head")) then {
            _a = _target getVariable["kat_aceAirway_airway_item", ""];
            _text = format ["STR_kat_aceAirway_%1_Display", _a];
            _genericMessages pushback ["localize _text", [0.08, 0.5, 0.72, 1]];
        };
        // END of EDIT
