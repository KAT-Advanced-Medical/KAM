...
private _bodyPartName = [
    ...
    LSTRING(Torso),
    LSTRING(ArmLeft),
    LSTRING(ArmUpperLeft),
    LSTRING(ArmRight),
    LSTRING(ArmUpperRight),
    LSTRING(LegLeft),
    LSTRING(LegUpperLeft),
    LSTRING(LegRight),
    LSTRING(LegUpperRight)
] select _selectionN;
    ...
    if (_bodyPartDamage > 0) then {
        ...
        switch (true) do {
            case (_bodyPartDamage isEqualTo 1): {
                _entries pushBack [localize ACELSTRING(medical_gui,traumaSustained4), [_bodyPartDamage] call FUNC(damageToRGBA)];
            };
            case (_bodyPartDamage >= 0.75): {
                _entries pushBack [localize ACELSTRING(medical_gui,traumaSustained3), [_bodyPartDamage] call FUNC(damageToRGBA)];
            };
            case (_bodyPartDamage >= 0.5): {
                _entries pushBack [localize ACELSTRING(medical_gui,traumaSustained2), [_bodyPartDamage] call FUNC(damageToRGBA)];
            };
            case (_bodyPartDamage >= 0.25): {
                _entries pushBack [localize ACELSTRING(medical_gui,traumaSustained1), [_bodyPartDamage] call FUNC(damageToRGBA)];
            };
        };
    };
};
