...
if (_part > 1 && ACEGVAR(medical,useLimbDamage) == 0) exitWith { false };

scopeName "main";

if (ACEGVAR(medical,fatalDamageSource) in [0, 2]) then {
    ...
    if (_part == 1 && {_woundDamage >= HEAD_DAMAGE_THRESHOLD}) exitWith {
        // Fatal damage to the head is guaranteed death
        TRACE_1("determineIfFatal: lethal headshot",_woundDamage);
        true breakOut "main";
    };
    if (_part == 2 && {_woundDamage >= ORGAN_DAMAGE_THRESHOLD} && {random 1 < HEART_HIT_CHANCE}) exitWith {
...
if (ACEGVAR(medical,fatalDamageSource) in [1, 2]) then {
