        ...
        [{
            ...
            private _occlusionState = _unit getVariable [QGVAR(occlusion), [0, 0, 0]] select _level;
                if (_occlusionState > 0) then {
                    ...
                    if ((floor (random 100) < GVAR(deterioratingAirway_chance))) then {
                        _occlusionTarget = (_occlusionState + 1) min 6;
                        _occlusionState set [_level, _occlusionTarget];
                        _unit setVariable [QGVAR(occlusion), _occlusionState, true];
                    };
                };
        }, GVAR(deterioratingOcclusion_interval), [_unit, _level]] call CBA_fnc_addPerFrameHandler;
    };
