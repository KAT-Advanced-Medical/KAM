...
if (_add) then {
	...
	if (_newValue > 65) then {[_unit, false, 0, true] call ace_medical_fnc_setUnconscious};
    _unit setVariable ["KAT_medical_airwayStatus", _newValue, true];
} else {
    _newValue = _oldValue - _value;
    if (_newValue < 65) then {_newValue = 65; [_unit, true] call ace_medical_fnc_setUnconscious};
    _unit setVariable ["KAT_medical_airwayStatus", _newValue, true];
};
